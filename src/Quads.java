...
public class Quads extends Application{
	...
	public void main(Stage s2) {
		...
		// box = new Group(); //horizontal box in which the rectangles are placed. seperated by 25 pixels
		...
		// box.setHgrow(rectangle, Priority.ALWAYS);
		...
		primaryStack.setBackground(new Background(new BackgroundFill(Color.BLACK, CornerRadii.EMPTY, Insets.EMPTY)));
		...
		task = new TimerTask() {
			...
			// long sec = 0;
					public void run() {
						Platform.runLater(() -> {
							// if (sec % time == 0) {
							// long um = time - (sec %time);
							// System.out.println(um);
							// if (um % time == 0) {
								read(predetermined, count, s2);
								count++;
							// } else if (um % (time/2l) == 0) {
							// 	primaryStack.getChildren().remove(old);
							// }
							// } else if (sec % (time/2l) == 0) {
							// 	primaryStack.getChildren().remove(old);
							// }
...
// 			}
	...
	public void blink(int num) {
		if (num == 1) { //correct one
			...
			// 	primaryStack.getChildren().remove(old);
			// }
			Shape a = new Circle(150);
			a.setFill(Color.WHITE);
			// box.getChildren().clear();
			primaryStack.getChildren().add(a);
			old = a;
			// rectangle.setShape(a);
			// rectangle.setBackground(new Background(new BackgroundFill(Color.WHITE, CornerRadii.EMPTY, Insets.EMPTY)));
			...
			// 	primaryStack.getChildren().remove(old);
			// }
			Shape b = new Rectangle(250, 250);
			b.setFill(Color.WHITE);
			// box.clear();
			
			primaryStack.getChildren().add(b);
			old = b;
			// rectangle.setShape(a);
			// rectangle.setBackground(new Background(new BackgroundFill(Color.WHITE, CornerRadii.EMPTY, Insets.EMPTY)));
			...
			c.getPoints().addAll(new Double[]{0.0,0.0, 250.0,0.0, 125.0,-200.0});
			c.setFill(Color.WHITE);
			// box.clear();
			primaryStack.getChildren().add(c);
			old = c;
			// rectangle.setShape(a);
			// rectangle.setBackground(new Background(new BackgroundFill(Color.WHITE, CornerRadii.EMPTY, Insets.EMPTY)));
			...
			// 	primaryStack.getChildren().remove(old);
			// }
			// Shape d = new Circle(100);
			Polygon d = new Polygon();
			// d.getPoints().addAll(new Double[]{12.0,1.0, 48.0,1.0, 71.0,35.0, 30.0,60.0, 1.0,35.0});
			d.getPoints().addAll(new Double[]{50.0,5.0, //bottom left
				200.0,5.0, //bottom right
				245.0,-140.0, // top right
				125.0,-240.0, // top most center
				5.0,-140.0}); // top left
			// double r = 100.0;
			// for(double i = 0.0; i < 5.0; i = i+ 1.0) {
			// 	d.getPoints().add(r*Math.cos(i*Math.toRadians(540.0)), r*Math.sin(i*Math.toRadians(540.0)));
			// }
			d.setFill(Color.WHITE);
			// box.clear();
			primaryStack.getChildren().add(d);
			// rectangle.setShape(a);
			// rectangle.setBackground(new Background(new BackgroundFill(Color.WHITE, CornerRadii.EMPTY, Insets.EMPTY)));
	...
	public  void writer(String id) {
		try {
			...
			file.write("End at " +(count + 1) + newline + newline);
			file.close();
		} catch(Exception e) {
